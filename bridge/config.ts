...
export function loadConfig(): Config {
  ...
  // The operator's config dir contains `.env` and the three operator-owned TOML inputs. The Herdr
  // plugin supervisor injects the exact directory; this fallback matches Web Remote's normal
  // owner-local config layout for direct bridge test and development starts.
  const configDir =
