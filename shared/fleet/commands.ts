...
function ordinalCommands(
  prefix: "select-tab" | "select-agent",
  noun: "Tab" | "Agent",
): FleetCommandDefinition[] {
...
export const FLEET_PUBLIC_DEFAULT_BINDINGS: Readonly<
...
const SPECIAL_KEYS = new Map<
  string,
  { code: string; label: string; impliedShift?: boolean }
>([
...
function exactKeys(
  ...
  if (extras.length)
    throw new Error(`${label} contains unknown field(s): ${extras.join(", ")}`);
}
...
export function parseFleetKeyChord(
  ...
  if (tokens.some((token) => !token))
    throw new Error(`${label} contains an empty key token`);
  ...
  const modifiers = {
    ...
    ctrlKey: false,
    metaKey: false,
    shiftKey: Boolean(key.impliedShift),
  };
  for (const token of tokens.slice(0, -1)) {
    const modifier = MODIFIER_LABELS.get(token.toLowerCase());
    if (!modifier)
      throw new Error(
        `${label} contains unsupported modifier ${JSON.stringify(token)}`,
      );
    const property =
      modifier === "Alt"
        ? "altKey"
    ...
    if (modifiers[property])
      throw new Error(`${label} repeats modifier ${modifier}`);
...
export function fleetChordSignature(
  chord: Pick<
    FleetKeyChord,
...
function parseBinding(
  commandId: string,
  value: unknown,
): FleetEffectiveBinding {
  if (typeof value !== "string")
    throw new Error(`bindings.${commandId} entries must be strings`);
  const prefix = value.startsWith("Prefix+");
  const chordText = prefix ? value.slice("Prefix+".length) : value;
  if (!chordText || chordText.includes("Prefix"))
    throw new Error(`bindings.${commandId} contains an invalid Prefix binding`);
  ...
  if (
    ...
    throw new Error(
      `bindings.${commandId} cannot bind bare Prefix+Escape because Escape cancels prefix mode`,
    );
  }
  return {
    commandId,
    kind: prefix ? "prefix" : "direct",
    chord,
    label: prefix ? `Prefix ${chord.label}` : chord.label,
  };
}
...
export function parseFleetShortcutDocument(
  ...
  if (record.schemaVersion !== FLEET_SHORTCUT_SCHEMA_VERSION) {
    throw new Error(
      `shortcut config schemaVersion must be ${FLEET_SHORTCUT_SCHEMA_VERSION}`,
    );
  }
  ...
  const unknownIds = Object.keys(rawBindings).filter(
    (id) => !COMMAND_IDS.has(id),
  );
  if (unknownIds.length)
    throw new Error(
      `shortcut config contains unknown command id(s): ${unknownIds.join(", ")}`,
    );
  if (options.requireComplete) {
    const missingIds = FLEET_COMMANDS.map((command) => command.id).filter(
      (id) => !(id in rawBindings),
    );
    if (missingIds.length)
      throw new Error(
        `shortcut config omits command id(s): ${missingIds.join(", ")}`,
      );
  }
  ...
  for (const command of FLEET_COMMANDS) {
    ...
    if (
      ...
      configured.length > MAX_BINDINGS_PER_COMMAND
    ) {
      throw new Error(
        `bindings.${command.id} must be an array with at most ${MAX_BINDINGS_PER_COMMAND} entries`,
      );
    }
    const commandBindings = configured.map((entry) =>
      parseBinding(command.id, entry),
    );
    for (const binding of commandBindings) {
      ...
      if (owner)
        throw new Error(
          `shortcut binding collision between ${owner} and ${command.id}: ${binding.label}`,
        );
  ...
  if (bindings.length > MAX_TOTAL_BINDINGS)
    throw new Error(`shortcut config exceeds ${MAX_TOTAL_BINDINGS} bindings`);
  const prefixCollision = signatures.get(
    `direct|${fleetChordSignature(prefix)}`,
  );
  if (prefixCollision)
    throw new Error(
      `shortcut prefix collides with direct binding for ${prefixCollision}`,
    );
  return {
...
function pureModifier(code: string): boolean {
  return /^(?:Alt|AltLeft|AltRight|Control|ControlLeft|ControlRight|Meta|MetaLeft|MetaRight|Shift|ShiftLeft|ShiftRight)$/.test(
    code,
  );
}
...
export function createFleetShortcutRecognizer(
  ...
  for (const binding of configuration.bindings) {
    (binding.kind === "direct" ? direct : prefixed).set(
      fleetChordSignature(binding.chord),
      binding,
    );
  }
  ...
  const boundedTimeout = Math.max(
    1,
    Math.min(timeoutMs, FLEET_SHORTCUT_PREFIX_TIMEOUT_MS),
  );
  const cancel = () => {
    pendingSince = null;
  };
  return {
    handle(event) {
      ...
      const current = now();
      if (pendingSince !== null && current - pendingSince >= boundedTimeout)
        pendingSince = null;
      ...
      if (pendingSince !== null) {
        ...
        return binding
          ? {
              ...
              bindingLabel: `${configuration.prefix.label} ${binding.chord.label}`,
            }
          : { kind: "cancelled" };
      }
