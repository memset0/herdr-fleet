...
function keys(
  ...
  if (extra.length)
    throw new Error(`${label} contains unknown field(s): ${extra.join(", ")}`);
}
...
function parseDiscordNotifications(
  value: unknown,
): DiscordNotificationConfig | undefined {
  if (value === undefined) return undefined;
  const raw = object(value, "discordNotifications");
  keys(
    raw,
    ["enabled", "executable", "channel", "template"],
    "discordNotifications",
  );
  if (typeof raw.enabled !== "boolean")
    throw new Error("discordNotifications.enabled must be a boolean");

  const executable =
    raw.executable === undefined
      ? undefined
      : absolutePath(raw.executable, "discordNotifications.executable");
  const channel = optionalSelector(
    raw.channel,
    "discordNotifications.channel",
    128,
  );
  const template = optionalSelector(
    raw.template,
    "discordNotifications.template",
    4_096,
  );
  if (
    channel &&
    !/^(?:[0-9]{1,32}|(?=[A-Za-z0-9_-]{1,64}$)(?=.*[A-Za-z])[A-Za-z0-9][A-Za-z0-9_-]*)$/.test(
      channel,
    )
  ) {
    throw new Error(
      "discordNotifications.channel must be a numeric id or configured alias",
    );
  }
  if (raw.enabled && (!executable || !channel)) {
    throw new Error(
      "enabled discordNotifications require executable and channel",
    );
  }
  return raw.enabled
    ? {
...
function parseFleetUi(value: unknown): FleetUiConfig {
  const shortcuts = parseFleetShortcutDocument(publicFleetShortcutDocument(), {
  ...
  const shortcutsFile =
    raw.shortcutsFile === undefined
      ? undefined
      : absolutePath(raw.shortcutsFile, "fleetUi.shortcutsFile");
  return {
    iframeCacheSize: integer(
      raw.iframeCacheSize ?? 1,
...
function parseLocalTransport(
  ...
  if (
    ...
    throw new Error(
      `${label}.url must contain only scheme, loopback host, and port`,
    );
  }
...
function sshEndpoint(
  ...
  if (validateKeys)
    keys(
      raw,
      ["host", "user", "port", "identityFile", "knownHostsFile"],
      label,
    );
  const host = hostname(raw.host, `${label}.host`);
  if (host.startsWith("-"))
    throw new Error(`${label}.host must not start with '-'`);
  const user = string(raw.user, `${label}.user`);
  if (!/^[A-Za-z_][A-Za-z0-9_.-]{0,63}$/.test(user))
    throw new Error(`${label}.user is invalid`);
  return {
    host,
    user,
...
function parseSshTransport(raw: JsonObject, label: string): SshTransportConfig {
  ...
  const jump =
    raw.jump === undefined
      ? undefined
      : sshEndpoint(object(raw.jump, `${label}.jump`), `${label}.jump`);
  if (jump?.identityFile === target.identityFile) {
    throw new Error(
      `${label}.jump.identityFile must differ from the target identity`,
    );
  }
...
function parseNode(
  ...
  keys(
    raw,
    ["id", "name", "publicHost", "enabled", "labels", "transport"],
    label,
  );
  ...
  if (
    ...
    rawLabels.some((entry) => typeof entry !== "string" || !entry.trim())
  ) {
...
export function parseGatewayConfig(value: unknown): GatewayConfig {
  ...
  keys(
    publicConfig,
    ["scheme", "fleetHost", "baseDomain", "cookieName", "sessionTtlSeconds"],
    "public",
  );
  if (publicConfig.scheme !== "https")
    throw new Error("public.scheme must be https");
  ...
  if (fleetHost !== baseDomain && !fleetHost.endsWith(`.${baseDomain}`)) {
    throw new Error(
      "public.fleetHost must equal or be below public.baseDomain",
    );
  }
  const cookieName =
  ...
  if (!/^[A-Za-z0-9_-]{1,64}$/.test(cookieName))
    throw new Error("public.cookieName is invalid");
  ...
  if (!/^[A-Za-z0-9_.-]{3,64}$/.test(username))
    throw new Error("auth.username is invalid");
  const passwordHash = string(auth.passwordHash, "auth.passwordHash");
  if (!passwordHash.startsWith("$argon2id$"))
    throw new Error("auth.passwordHash must be Argon2id");
  ...
  if (
    ...
    throw new Error(
      "auth.sessionSecret must be at least 32 random base64url bytes",
    );
  }

  if (!Array.isArray(raw.nodes) || raw.nodes.length === 0)
    throw new Error("nodes must contain at least one instance");
  const nodes = raw.nodes.map((node, index) =>
    parseNode(node, index, baseDomain),
  );
  if (!nodes.some((node) => node.enabled))
    throw new Error("nodes must contain at least one enabled instance");
  ...
  for (const node of nodes) {
    ...
    if (hosts.has(node.publicHost) || node.publicHost === fleetHost)
      throw new Error(`duplicate public host: ${node.publicHost}`);
    ids.add(node.id);
    ...
    if (node.transport.type === "ssh") {
      if (node.enabled) {
        ...
        if (existingNode) {
          throw new Error(
            `duplicate SSH identity path: ${node.transport.identityFile} (${existingNode}, ${node.id})`,
          );
        }
        sshIdentityPaths.set(node.transport.identityFile, node.id);
      }
      if (localPorts.has(node.transport.localPort))
        throw new Error(
          `duplicate local listener port: ${node.transport.localPort}`,
        );
      localPorts.add(node.transport.localPort);
    } else {
      const upstreamPort = Number(new URL(node.transport.url).port);
      if (localPorts.has(upstreamPort))
        throw new Error(`duplicate local listener port: ${upstreamPort}`);
      localPorts.add(upstreamPort);
    }
  }

  const discordNotifications = parseDiscordNotifications(
  ...
  return {
    ...
    public: {
      ...
      sessionTtlSeconds: integer(
        publicConfig.sessionTtlSeconds ?? 604_800,
        "public.sessionTtlSeconds",
        300,
        2_592_000,
      ),
    },
...
export async function loadGatewayConfig(
  ...
  if (!isAbsolute(path))
    throw new Error("gateway config path must be absolute");
  if (enforcePermissions) {
    const info = await stat(path);
    if ((info.mode & 0o077) !== 0)
      throw new Error(
        "gateway config must not be accessible by group or other users (chmod 600)",
      );
  }
  let config = parseGatewayConfig(
  ...
  if (!shortcutInfo?.isFile()) {
    throw new Error(
      config.fleetUi.shortcutsFile
        ? "fleetUi.shortcutsFile is unavailable or is not a regular file"
        : "packaged Fleet shortcut defaults are unavailable",
    );
  }
  if (shortcutInfo.size > 1_048_576)
    throw new Error("Fleet shortcut config exceeds 1 MiB");
  let shortcutValue: unknown;
  try {
    shortcutValue = JSON.parse(await shortcutSource.text()) as unknown;
  } catch (error) {
    throw new Error(
      `Fleet shortcut config is not valid JSON: ${error instanceof Error ? error.message : String(error)}`,
    );
  }
  ...
  if (config.discordNotifications?.enabled) {
    const executable = await stat(config.discordNotifications.executable).catch(
      () => null,
    );
    if (!executable)
      throw new Error("discordNotifications.executable is unavailable");
    if (!executable.isFile() || (executable.mode & 0o111) === 0) {
      throw new Error(
        "discordNotifications.executable must be a regular executable file",
      );
    }
  }
  ...
  async function identityDigest(
    ...
    if (!identity.isFile())
      throw new Error(`${label} identity must be a regular file`);
    if (!knownHosts.isFile())
      throw new Error(`${label} known-hosts path must be a regular file`);
    if (enforcePermissions && (identity.mode & 0o077) !== 0) {
      throw new Error(
        `${label} identity must not be accessible by group or other users (chmod 600)`,
      );
    }
    return createHash("sha256")
      .update(await readFile(identityPath))
      .digest("base64url");
  }
  ...
  for (const node of config.nodes) {
    ...
    if (node.enabled) {
      ...
      if (existingNode)
        throw new Error(
          `${node.id} reuses the SSH private identity assigned to ${existingNode}`,
        );
    ...
    if (node.transport.jump) {
      ...
      if (jumpDigest === digest)
        throw new Error(
          `${node.id} SSH jump reuses its target private identity`,
        );
  ...
  for (const [digest, jumpNode] of jumpIdentityDigests) {
    ...
    if (targetNode) {
      throw new Error(
        `${jumpNode} SSH jump reuses the target private identity assigned to ${targetNode}`,
      );
    }
  }
  return config;
