...
async function packagedDefaults(): Promise<unknown> {
  return JSON.parse(
    await Bun.file(new URL("./shortcuts.default.json", import.meta.url)).text(),
  ) as unknown;
}
...
async function packagedExternalExample(): Promise<unknown> {
  return JSON.parse(
    await Bun.file(
      new URL("./shortcuts.external.example.json", import.meta.url),
    ).text(),
  ) as unknown;
}
...
describe("Fleet command catalog", () => {
  test("contains every stable command exactly once and complete public defaults", async () => {
    ...
    expect(new Set(FLEET_COMMANDS.map((command) => command.id)).size).toBe(
      FLEET_COMMANDS.length,
    );
    expect(new Set(FLEET_COMMANDS.map((command) => command.name)).size).toBe(
      FLEET_COMMANDS.length,
    );
    expect(Object.keys(FLEET_PUBLIC_DEFAULT_BINDINGS).sort()).toEqual(
      FLEET_COMMANDS.map((command) => command.id).sort(),
    );
    ...
    expect(
      parsed.bindingsByCommand["create-tab"]?.map((binding) => binding.label),
    ).toEqual(["Prefix C", "Prefix V", "Prefix -"]);
  });

  test("keeps the synthetic external example and Gateway example aligned with the parser", async () => {
    const example = await packagedExternalExample();
    const parsed = parseFleetShortcutDocument(example, {
      requireComplete: true,
    });
    expect(parsed.bindingsByCommand["copy-fleet-pane-link"]).toEqual([]);
    expect(
      parsed.bindingsByCommand["toggle-type-mode"]?.map(
        (binding) => binding.label,
      ),
    ).toEqual(["Prefix T"]);

    const gatewayExample = (await Bun.file(
      new URL("../gateway.example.json", import.meta.url),
    ).json()) as {
      fleetUi?: { shortcutsFile?: unknown };
    };
    expect(gatewayExample.fleetUi?.shortcutsFile).toBe(
      "/etc/herdr-web-remote/shortcuts.json",
    );
  });
  ...
  test("rejects malformed, unknown, incomplete, colliding, and payload-bearing documents", () => {
    ...
    expect(() =>
      parseFleetShortcutDocument({ ...complete, schemaVersion: 2 }),
    ...
    expect(() =>
      parseFleetShortcutDocument({
        ...complete,
        bindings: { ...complete.bindings, "not-a-command": [] },
      }),
    ...
    expect(() =>
      parseFleetShortcutDocument(missing, { requireComplete: true }),
    ).toThrow("omits command");
    expect(
      parseFleetShortcutDocument(missing).bindingsByCommand["last-pane"],
    ).toEqual([]);
    ...
    expect(() => parseFleetShortcutDocument(prefixCollision)).toThrow(
      "prefix collides",
    );
    expect(() =>
      parseFleetShortcutDocument({ ...complete, prefix: "Ctrl+Ctrl+B" }),
    ).toThrow("repeats modifier");
    expect(() =>
      parseFleetShortcutDocument({ ...complete, prefix: "Ctrl+F12" }),
...
describe("Fleet direct/prefix shortcut recognizer", () => {
  test("distinguishes sequential prefix chords from simultaneous direct chords", () => {
    ...
    expect(
      recognizer.handle(key("KeyP", { ctrlKey: true, shiftKey: true })),
    ).toEqual({
      kind: "command",
      commandId: "open-command-palette",
      bindingLabel: "Ctrl+Shift+P",
    });
    expect(recognizer.handle(key("KeyB", { ctrlKey: true }))).toEqual({
      kind: "prefix",
    });
    expect(recognizer.pending()).toBe(true);
    expect(recognizer.handle(key("ShiftLeft", { shiftKey: true }))).toEqual({
      kind: "ignored",
    });
  ...
  test("cancels prefix on Escape, unsupported keys, extra modifiers, and explicit context loss", () => {
    ...
    expect(recognizer.handle(key("KeyS"))).toEqual({ kind: "ignored" });
    expect(
      recognizer.handle(key("KeyJ", { altKey: true, repeat: true })),
    ).toEqual({ kind: "ignored" });
  });

  test("uses exact physical-code signatures", () => {
    expect(fleetChordSignature(parseFleetKeyChord("Alt+J"))).toBe(
      "KeyJ|true|false|false|false",
    );
    expect(fleetChordSignature(parseFleetKeyChord("Ctrl+Shift+P"))).toBe(
      "KeyP|false|true|false|true",
    );
  });
});
