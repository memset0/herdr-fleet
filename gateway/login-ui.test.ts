...
import { APP_CSS, FLEET_CSS, GATEWAY_THEME_CSS, loginPage } from "./ui.ts";
...
describe("Gateway login UI", () => {
  test("renders the script-free single-account form with explicit accessible labels", () => {
    ...
    expect(page).toContain('<section class="login-panel" aria-labelledby="login-title">');
    expect(page).toContain('<h1 id="login-title">Sign in</h1>');
    expect(page).toContain("Herdr Web Remote");
    expect(page).toContain('<form method="post" action="/auth/login" autocomplete="on" aria-describedby="login-description">');
    expect(page).toContain('<input type="hidden" name="next" value="https://fleet.example.com/?instance=node-a">');
    expect(page).toContain('<label for="username">Username</label>');
    expect(page).toContain('id="username" name="username" type="text" autocomplete="username" maxlength="64" required autofocus');
    expect(page).toContain('<label for="password">Password</label>');
    expect(page).toContain('id="password" name="password" type="password" autocomplete="current-password" maxlength="512" required');
    expect(page).toContain('<button class="primary-action" type="submit">Sign in</button>');
  ...
  test("escapes the generic alert and validated return value without echoing fields", () => {
    ...
    expect(page).toContain("&lt;img src=x onerror=&quot;message-leak&quot;&gt; Invalid username or password.");
    expect(page).toContain("https://fleet.example.com/&quot;&gt;&lt;img src=x onerror=&quot;return-leak&quot;&gt;");
  ...
  test("uses the shared adaptive theme without the retired ornamental treatment", () => {
    ...
    expect(FLEET_CSS.startsWith(GATEWAY_THEME_CSS)).toBeTrue();
    ...
    expect(GATEWAY_THEME_CSS).toContain(":focus-visible { outline: 2px solid var(--ring)");
    ...
    expect(FLEET_CSS.indexOf(GATEWAY_THEME_CSS)).toBe(0);
  });
});
