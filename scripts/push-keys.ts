...
// to go looking for". Run it explicitly against Web Remote's device-local config:
//   bun scripts/push-keys.ts "$(herdr plugin config-dir memset0.web-remote)/.env" \
...
export function mergeEnv(text: string, vars: Record<string, string>): string {
  ...
  if (appended.length > 0) {
    ...
    lines.push("", "# --- Web Push (VAPID) — written by `scripts/push-keys.ts` ---", ...appended);
  }
 ...
 * through an allowlist so the device-local .env has one portable interpretation and never gains
 * control characters or shell metacharacters. The set below covers ordinary mailto/https subjects.
 */
...
if (import.meta.main) {
  ...
  if (already && !force && !subjectOnly) {
    console.error(
      `✗ push keys are already configured in ${envPath}\n` +
        ...
        `  If that's what you want: \`bun scripts/push-keys.ts ${envPath} --force\`.\n` +
  ...
  console.log("\nNext:");
  console.log("  1. herdr plugin action invoke restart --plugin memset0.web-remote");
  console.log("  2. On your phone: open Collie → Settings → enable notifications");
  console.log("  3. Use the existing notification test surface to verify delivery");
}
