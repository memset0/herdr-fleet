...
exists to prevent. The shapes it refuses SHALL include a hostname outside the reserved example and
loopback names, an IP address outside loopback and the documentation ranges, an absolute path under a
user's home directory, and material shaped like a private key, a password verifier or a long opaque
secret.

Where a private name has no shape — a machine whose name is an ordinary word — the check SHALL read
those names from the repository's ignored local context file when it is present, so they are caught
without any of them being written into the tree.
...
- **WHEN** a tracked file gains a real hostname, a non-documentation address, a home-directory path, or credential-shaped material
...
- **WHEN** a tracked file gains a private machine name that is an ordinary word, and the local context file names it
