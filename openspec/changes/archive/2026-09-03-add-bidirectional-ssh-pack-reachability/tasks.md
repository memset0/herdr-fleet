...
- [x] 6.2 Commit the complete feature as one conventional commit with the required trailers, fetch and verify `origin/v3-dev`, push normally, and verify the remote exact commit.
- [x] 6.3 Deploy only the exact pushed candidate to existing isolated Lead staging with its unchanged configuration and the existing controller; verify readiness, that the Lead's behavior is unchanged with no reachability list configured, and that trust, configuration, STT, Push, v2 and Herdr state are untouched, then update deployment state.
