...
state and MUST NOT navigate, mutate a Pane, refresh, revalidate, or issue an API request.

A level that holds exactly one child SHALL be elided: the child is presented in its place, the
deeper name and icon win, and the elided level MUST NOT contribute a row, a disclosure control, or
an indentation step. A row SHALL present a group icon only when it still groups more than one child
...
- **THEN** that level is not drawn as its own row and its single child appears at the elided level under the child's own name and icon
