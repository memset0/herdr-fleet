...
- [x] 9.1 Exercise a peer Pane end to end through the lead's Gateway — open, type, resize, leave, return — and record that the peer's terminal id never appeared on the lead outside the peer's own reply — **run against a real member service and a real terminal server over loopback**, which stands in for the SSH hop the projection test pins separately: the member resolved its own Pane and started one terminal server, the screen arrived, typing worked, the Pane's rows went 50 → 30 and back to 50 on a resize, a return inside the grace period re-used the session with no new terminal server and replayed the retained window, and the lead's own `stop` reached the member's `close`. The lead's diagnostics name the Pane and the machine and carry no terminal id at any point.
...
- [x] 9.3 Verify the Pane, its terminal, the multiplexer server, Collie, and the link are unchanged after a full terminal lifecycle on a peer — after the run above the Pane is at the rows it started at, no terminal server remains, and nothing else on the machine was touched. The one thing this run does not cover is the SSH hop itself, which needs a member carrying a `[terminal]` table — deployment work this change does not own.
...
- [x] 10.3 Verify `FORK.toml` records every upstream path this change touched with its reason, and that `web/src/components/agent-chat.tsx` is not among them, via `bun scripts/check-fork.ts` — the change's non-owned paths are `web/src/router.tsx` (its own new entry), the seven typed dictionaries (already carried by `native-agent-favorites-port`), and `web/package.json` + `web/bun.lock` (`downstream-version-line`). `agent-chat.tsx` is untouched by this change.
...
- [x] 10.6 Run the full gate once when commit-ready: `bun test`, `cd web && bun run test`, both typechecks, `bun run lint`, `bun scripts/check-fork.ts`, `bash scripts/check-version.sh`, `bun run build`, `openspec validate attach-the-browser-to-the-real-terminal --strict`, `git diff --check` — run on `nvl72-cluster`, where the suites finish; `openspec validate` runs on the development host because the CLI is not installed there. The root suite's `bun test ./bridge` is run file by file under a timeout, because two files under `bridge/stt` install fake timers and their runner does not exit afterwards — a defect that arrived with the Collie 1.5.2 adoption and is absent at its parent commit, and is not this change's.
