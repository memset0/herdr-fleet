...
- `AGENTS.md` is the canonical instruction file. `CLAUDE.md` MUST remain a relative symbolic link
  to it so every agent reads the same rules.
- This repository is a staged downstream reapplication of Collie. Preserve upstream behavior by
  default; source-repository specifications describe only downstream additions, changes, and fork
  boundaries, never Collie features that remain unchanged.
- Use one OpenSpec change for each coherent reapplication stage. Proposal, apply, validation, sync,
  archive, commit, and push remain separate gates, but successful proposals automatically proceed
  to apply and verified implementations automatically proceed to archive unless the current user
  request explicitly asks to stop for review.
...
- Put downstream behavior in explicit fork-owned modules declared by `FORK.toml`. Upstream-owned
  files may expose narrow interfaces, lifecycle hooks, slots, or data ports, but MUST NOT absorb
  downstream business logic when an owned module can carry it.
- Reuse Collie's native components, data contracts, and settings architecture before adding a fork
  abstraction. Preserve interaction semantics during reapplication while allowing the internal
  implementation to follow current Collie structure.
- Add only focused tests required to protect downstream behavior and invasive ports. Keep existing
  upstream coverage and avoid duplicating tests for unchanged Collie behavior.
- The tracked tree is public-safe. Never add private deployment names, infrastructure topology,
  credentials, parent-repository paths, personal identifiers, or machine-local state to source,
  tests, fixtures, docs, OpenSpec artifacts, commit messages, or Git history.
- `LOCAL.md` is ignored local operator context. Read it when present for deployment facts, but never
  quote, summarize, or transfer its private content into tracked files.
- The stable downstream plugin identifier is `memset0.herdr-fleet`. Any identity migration belongs
  to an explicit OpenSpec change and MUST keep packaging, runtime registration, documentation, and
  compatibility decisions aligned.
