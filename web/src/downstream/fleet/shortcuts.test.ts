...
class FakeShortcutEnvironment implements FleetShortcutEnvironment {
  ...
  readonly posted: Array<
    FleetShortcutIntent | FleetShortcutReady | FleetShortcutResult
  > = [];
  hidden = false;
  ids = 0;
  ...
  addKeyListener = (listener: (event: KeyboardEvent) => void) =>
    this.keys.add(listener);
  removeKeyListener = (listener: (event: KeyboardEvent) => void) =>
    this.keys.delete(listener);
  addCancelListener = (listener: () => void) => this.cancels.add(listener);
  removeCancelListener = (listener: () => void) =>
    this.cancels.delete(listener);
  postParent = (
...
describe("Fleet framed shortcut bridge", () => {
  it("parses only strict v2 configuration and fixed allowlisted commands", () => {
    ...
    expect(
      parseFleetShortcutConfig({ ...config, url: "/pane/private" }),
    ).toBeNull();
    expect(
      parseFleetShortcutConfig({
        ...config,
        prefix: { ...config.prefix, code: "F12", label: "F12" },
      }),
    ).toBeNull();
    expect(
      parseFleetShortcutConfig({
    ...
    expect(
      parseFleetShortcutConfig({
        ...config,
        bindings: [{ ...config.bindings[0], commandId: "unknown" }],
      }),
    ).toBeNull();
    expect(parseFleetShortcutCommand(command)).toEqual(command);
    expect(
      parseFleetShortcutCommand({ ...command, action: "terminal-input" }),
    ).toBeNull();
    expect(
      parseFleetShortcutCommand({ ...command, keys: ["Escape"] }),
    ).toBeNull();
    expect(
      parseFleetShortcutCommand({ ...command, requestId: "short" }),
    ).toBeNull();
    expect(
      parseFleetShortcutCommand({ ...command, generation: -1 }),
    ).toBeNull();
  });
  ...
  it("recognizes direct and sequential prefix chords including Shift and Tab variants", () => {
    ...
    expect(
      environment.key({ code: "KeyP", ctrlKey: true, shiftKey: true })
        .defaultPrevented,
    ).toBe(false);
    ...
    expect(
      environment.key({ code: "KeyB", ctrlKey: true }).defaultPrevented,
    ).toBe(true);
    expect(
      environment.key({ code: "ShiftLeft", shiftKey: true }).defaultPrevented,
    ).toBe(false);
    expect(
      environment.key({ code: "KeyP", shiftKey: true }).defaultPrevented,
    ).toBe(true);
    expect(
      environment.key({ code: "KeyB", ctrlKey: true }).defaultPrevented,
    ).toBe(true);
    ...
    expect(
      environment.key({ code: "Tab", shiftKey: true }).defaultPrevented,
    ).toBe(true);
  ...
  it("cancels prefix on context loss, hidden state, unsupported input, and newer generations", () => {
    ...
    expect(
      environment.key({ code: "KeyJ", altKey: true }).defaultPrevented,
    ).toBe(false);
  ...
  it("does not install bridge behavior in standalone Collie", () => {
    ...
    environment.message(environment.parent, config);
    expect(
      environment.key({ code: "KeyS", altKey: true }).defaultPrevented,
    ).toBe(false);
  });
  ...
  it("runs a fixed handler once and deduplicates correlated request ids", async () => {
    ...
    createFleetShortcutController(
      environment,
      new Map([["fit-pane-width", handler]]),
    )();
    environment.message(environment.parent, config);
    ...
    await vi.waitFor(() =>
      expect(
        environment.posted.filter(
          (message) => message.type === FLEET_SHORTCUT_RESULT_TYPE,
        ),
      ).toHaveLength(2),
    );
    expect(handler).toHaveBeenCalledTimes(1);
    const results = environment.posted.filter(
      (message): message is FleetShortcutResult =>
        message.type === FLEET_SHORTCUT_RESULT_TYPE,
    );
  ...
  it("rejects arbitrary key payloads and bounds throwing or timed-out handler errors", async () => {
    ...
    environment.message(environment.parent, {
      ...command,
      requestId: "request_escape_1",
      action: "send-escape",
    });
    environment.message(environment.parent, {
      ...command,
      requestId: "request_enter_12",
      action: "send-enter",
    });
    environment.message(environment.parent, {
      ...command,
      requestId: "request_keys_123",
      action: "send-enter",
      keys: ["x"],
    });
    await vi.waitFor(() =>
    ...
    const failures = environment.posted.filter(
      (message): message is FleetShortcutResult =>
        message.type === FLEET_SHORTCUT_RESULT_TYPE,
    );
