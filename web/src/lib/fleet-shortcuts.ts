...
export interface FleetShortcutEnvironment {
  ...
  postParent(message: FleetShortcutIntent | FleetShortcutReady | FleetShortcutResult): void;
  randomId(): string;
  now(): number;
}
...
const CODE = /^(?:Key[A-Z]|Digit[0-9]|Tab|Minus|Slash|Escape|Enter|Space|Arrow(?:Up|Down|Left|Right))$/;
const MAX_BINDINGS = 256;
const MAX_RESULTS = 64;
const COMMAND_TIMEOUT_MS = 15_000;
const CONFIG_KEYS = new Set(["type", "version", "generation", "active", "prefix", "bindings"]);
const CHORD_KEYS = new Set(["code", "altKey", "ctrlKey", "metaKey", "shiftKey", "label"]);
const BINDING_KEYS = new Set([...CHORD_KEYS, "commandId", "kind"]);
const COMMAND_KEYS = new Set(["type", "version", "generation", "requestId", "action"]);
...
function parseWireChord(value: unknown): FleetShortcutWireChord | null {
  ...
  if (!exactKeys(record, CHORD_KEYS) || !CODE.test(String(record.code))) return null;
  if ([record.altKey, record.ctrlKey, record.metaKey, record.shiftKey].some((part) => typeof part !== "boolean")) return null;
  if (typeof record.label !== "string" || !record.label || record.label.length > 64) return null;
  try {
    const parsed = parseFleetKeyChord(record.label);
    if (fleetChordSignature(parsed) !== fleetChordSignature(record as unknown as FleetShortcutWireChord)) return null;
  } catch {
    return null;
  }
...
function parseBinding(value: unknown): FleetShortcutBinding | null {
  if (!value || typeof value !== "object" || Array.isArray(value)) return null;
  const record = value as Record<string, unknown>;
  if (!exactKeys(record, BINDING_KEYS) || !isFleetCommandId(record.commandId)) return null;
  if (record.kind !== "direct" && record.kind !== "prefix") return null;
  const chord = Object.fromEntries(Object.entries(record).filter(([key]) => CHORD_KEYS.has(key)));
  return parseWireChord(chord) ? record as unknown as FleetShortcutBinding : null;
}
...
export function parseFleetShortcutConfig(value: unknown): FleetShortcutConfig | null {
  ...
  const signatures = valid.map((binding) => `${binding.kind}|${fleetChordSignature(binding)}`);
  if (new Set(signatures).size !== signatures.length) return null;
  if (valid.some((binding) => binding.kind === "direct" && fleetChordSignature(binding) === fleetChordSignature(prefix))) return null;
...
function recognizerConfiguration(config: FleetShortcutConfig): FleetShortcutConfiguration {
  ...
  const bindings: FleetEffectiveBinding[] = config.bindings.map((binding) => ({
    ...
    label: binding.kind === "prefix" ? `Prefix ${binding.label}` : binding.label,
  }));
  const bindingsByCommand: Record<string, FleetEffectiveBinding[]> = {};
  for (const binding of bindings) (bindingsByCommand[binding.commandId] ??= []).push(binding);
...
export function createFleetShortcutController(
  ...
  const contextActive = (expectedGeneration: number): boolean => (
    started && active && generation === expectedGeneration && !environment.documentHidden()
  );
  ...
  const waitForHandler = async (
    ...
    const deadline = environment.now() + Math.max(0, Math.min(handlerWaitMs, 5_000));
    let handler = contextActive(expectedGeneration) ? handlers.get(action) : undefined;
    while (!handler && contextActive(expectedGeneration) && environment.now() < deadline) {
  ...
  const runCommand = async (command: FleetShortcutCommand): Promise<FleetShortcutResult> => {
    ...
    return new Promise((resolve) => {
      ...
      void Promise.resolve().then(() => {
        if (!contextActive(command.generation)) throw new Error("Shortcut action is unavailable");
        return handler();
      }).then(
        () => finish({
          type: FLEET_SHORTCUT_RESULT_TYPE,
  ...
  const onMessage: MessageListener = (event) => {
    ...
    if (config) {
      ...
      recognizer = config.active
        ? createFleetShortcutRecognizer(recognizerConfiguration(config), environment.now)
        : null;
      environment.postParent({
        type: FLEET_SHORTCUT_READY_TYPE,
        version: FLEET_SHORTCUT_VERSION,
        generation,
        commands: [...new Set(config.bindings.map((binding) => binding.commandId))],
  ...
  const onKey: KeyListener = (event) => {
    if (!started || !active || !recognizer || environment.documentHidden()) return;
