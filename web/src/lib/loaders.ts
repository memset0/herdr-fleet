...
export async function rootLoader({ request }: { request?: Request } = {}): Promise<HomeData> {
  ...
  try {
    ...
    rememberAuthError(session, authError);
    if (authError) clearAuthorizedCaches();
...
function clearAuthorizedCaches(): void {
  // A definitive node auth refusal invalidates every persisted screen at this exact origin, not
  // merely the requested session. Clear module mirrors too so the same resident document cannot
  // keep presenting them after the durable tier has gone.
  clearLastSeen();
  lastSnapshot.clear();
  lastPaneText.clear();
}
