...
describe("rootLoader", () => {
  ...
  it("clears warm module and persisted snapshots before returning a 401", async () => {
    const { rootLoader } = await import("./loaders");
    await rootLoader();
    expect(sessionStorage.getItem("collie:last-snapshot:")).not.toBeNull();
    rejectSnapshot(401);

    const rejected = await rootLoader();
    expect(rejected.authError).toBe(true);
    expect(rejected.agents).toEqual([]);
    expect(sessionStorage.getItem("collie:last-snapshot:")).toBeNull();
  });
...
describe("paneLoader", () => {
  ...
  it("clears every warm persisted screen before returning a pane 403", async () => {
    ...
    const rejected = await paneLoader({ params: { paneId: "w1:p1" } });
    expect(rejected.authError).toBe(true);
    expect(rejected.text).toBe("");
    expect(sessionStorage.getItem("collie:last-snapshot:")).toBeNull();
    expect(sessionStorage.getItem("collie:last-pane: w1:p1")).toBeNull();
  });
